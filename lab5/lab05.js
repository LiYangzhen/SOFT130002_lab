...
function showWindowHref() {
    let indexPart = url.value.substring(url.value.indexOf("?") + 1);
    url_result.value = indexPart.substring(indexPart.indexOf("=") + 1, indexPart.indexOf("&"));
}
...
function timeTest() {
    ...
    timer = setInterval(mul_double, 5000);
}

function mul_double() {
    let myDate = new Date();
    if (minute !== myDate.getMinutes() || count > 10) {
        clearInterval(timer);
    } else {
        mul.value *= 2;
        count += 1;
...
function arrSameStr() {
    result.value = null;
    let mostString = most.value;
    let ob = {};
    let i = 0, length = mostString.length;
    let resultString = "";
    ...
    for (let j in ob) {
        if (ob[j] === max) {
            resultString += "The most character is:" + j + " times:" + max+"  ";
        }
    }
